...
// // gather results
// (
//   node["bridge"="yes"](30.5,105,40.5,106); // Bridge
//   way["highway"="primary"]({{bbox}}); // Highway
//   node["natural"="water"](30.5,105,40.5,106); // Body of water
...
export function loadGeoDataAsync(){
  ...
  var queryURL = 'https://www.overpass-api.de/api/interpreter?data=' + 
        '[out:json][timeout:60];' + 
        'node["bridge"="yes"](40,-105.5,40.1,-105.2);' +
  ...
  $.ajax({
    ...
    console.log(error);
    console.error( "Critical Error: Not able to load geo data" );
  }).always(function() {
