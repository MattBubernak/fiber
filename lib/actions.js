...
function forEachFiberSetCost(fiber){
  ...
  (distance, coord) =  _.reduce(fiber.geometry.coordinates, function(min,coordinate) { 
    let dist = geolib.getDistance(store.selectedPosition,coordinate) 
    if (dist < min) return dist
    else return min 
  },Number.MAX_VALUE);
  
  fiber.distance = distance[1]
  ...
  console.log("Elements", store.geoData['elements'])
  for( var x in store.geoData.elements){
    
    var hazard = store.geoData.elements[x]
    
    //x is the hazard object
    
    console.log("Type - ", hazard)
    
    if( isCloseHazard(store.selectedPosition, coord )){
      if( isWater(hazard)){
        hazards.append([hazard.name, waterCost]);
      }
      else if(isBridge(hazard)){
        hazards.append([hazard.name, bridgeCost]);
      }
      else if(isHighway(hazard)){
        hazards.append([hazard.name, highwayCost]);
      }else if(isSchool(hazard)){
        hazards.append([hazard.name, schoolCost]);
      }
    }
  }
  console.log("Done looping through hazards")
