...
export function setSelectedPosition(latlng) {
  if (store.selectOnlyOne) {
    ...
    store.selectedPosition = store.firstSelectedPosition
    if (store.geoData !== null) {
      store.isLoading = true;
      _.forEach(store.fibers, forEachFiberSetIsSelected)
      async(fiberCalc);
    ...
    store.selectedPosition = store.secondSelectedPosition
    if (store.geoData !== null) {
      store.isLoading = true;
      _.forEach(store.fibers, forEachFiberSetIsSelected)
      async(fiberCalc);
...
function calculateCost(fiber, hazards) {
  ...
  var discount = calculateFiberDiscountBetween(store.firstSelectedPosition, store.secondSelectedPosition)
  ...
  if(!store.selectOnlyOne){
      ...
      store.charge.append(cost);
    
    if( fiber.charge.count == 2){
      var first = store.charge[0]
      var second = store.charge[1]
